...
자체 제작 네트워크 라이브러리 기반의 고성능 MMORPG 분산 서버 시스템
...
네트워크 비동기 통신 코어 라이브러리를 바닥부터 직접 설계 및 구현하였으며, 이를 기반으로 실제 게임 환경에서 동작하는 로그인 및 채팅 서버를 구축했습니다. 대규모 트래픽 환경에서의 동시성 처리, I/O 성능 최적화, 그리고 분산 서버 간의 스케일 아웃(Scale-out) 확장성에 중점을 두고 개발하였습니다.
...
| **송신용 락프리 큐 (Lock-Free)** | CAS(Compare-And-Swap) 연산 기반으로 대기열을 구현하여, 직렬화 버퍼의 주소를 `WSASend`에 락 경합 없이 즉시 전달합니다. |
...
| **중복 로그인 제어** | 동일 계정의 다중 접속 시도를 감지하고 선제적으로 차단하여 동기화 이슈를 방지합니다. |
...
![전체 시스템 아키텍처](3.%20Integration%20%26%20Demo/Images/TotalServerFlow.png)
*(참고: 실제 이미지 경로에 맞게 괄호 안의 경로를 수정하여 사용해 주세요)*
