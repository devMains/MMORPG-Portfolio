...
네트워크 비동기 통신 코어 라이브러리를 직접 설계 및 구현하였으며, 이를 기반으로 로그인, 채팅, 게임 서버를 구축했습니다. 대규모 트래픽 환경에서의 동시성 처리, I/O 성능 최적화, 그리고 섹터 기반 AOI(Area of Interest)를 사용하는 게임 월드 동기화에 중점을 두고 개발하였습니다.
...
| **송신용 락프리 큐 (Lock-Free)** | CAS(Compare-And-Swap) 연산 기반으로 큐를 구현하여, 동기화 객체 없이 직렬화 버퍼의 주소를 즉시 전달합니다. |
...
### 2.4 게임 서버 (Game Server)

**[🔗 게임 서버 상세 문서](2.%20Servers/GameServer/README.md)**

| 핵심 기능 | 아키텍처 및 구현 특징 |
| :--- | :--- |
| **n × n 격자 월드 & 3×3 AOI** | 월드를 n × n 격자로 나누고, 플레이어 주변 3×3 섹터(Area of Interest)에 속한 캐릭터의 이동·공격 정보를 선택적으로 동기화합니다. 이는 실제 MMO에서 사용하는 공간 분할 기반 AOI 패턴과 유사한 구조입니다. |
| **단계적 멀티스레드 동기화** | ① 단일 스레드 대륙 기반 서버 → ② 스레드 간 이동·3×3 섹터 생성/삭제를 전파하는 심리스 서버 → ③ 다른 스레드 간 공격·피격까지 처리하는 완전 멀티 스레드 전투 서버 순으로 점진적으로 확장하며 동기화 문제를 해결했습니다. |
| **스레드/섹터 파티셔닝** | 섹터 그룹(대륙)을 스레드 단위로 파티셔닝하고, 스레드 간 경계에서의 세션 이동과 AOI 갱신을 메시지 패싱 방식으로 처리해 락 사용을 최소화했습니다. |
| **네트워크 라이브러리 확장** | 스레드 간 Group 메시지 전달, 그룹 이동 시 Pending 메시지 처리 보장, 섹터 단위 락 관리 등 게임 서버 요구사항에 맞춘 네트워크 라이브러리 변경점을 반영했습니다. |

## 3. 전체 시스템 아키텍처 (System Architecture)

**[🔗 서버 구조 및 통신 흐름 상세 문서](3.%20Integration%20%26%20Demo/ServerArchitecture.md)**

각각 독립적으로 구동되는 서버들이 Redis 글로벌 캐시와 유저 DB를 매개로 어떻게 안전하게 세션을 확립하고 통신하는지 보여주는 전체 조감도입니다. 또한, 게임 서버는 n × n 격자 기반 AOI 설계를 통해 로그인/채팅 서버와 연계된 형태로 실제 게임 월드의 이동·전투 동기화를 담당합니다.

![전체 시스템 아키텍처](./Images/TotalServerFlow.png)

**[🔗 서버 연결 흐름 더미 테스트 상세 문서](3.%20Integration%20%26%20Demo/Demo.md)**
