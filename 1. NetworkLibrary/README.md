...
*   **그룹 기반 실행 컨텍스트:** 인스턴스 던전·필드·PVP 존 등 **컨텐츠 단위로 독립된 그룹(Group)을 구성**하고, 그룹 단위로 메시지/로직을 직렬화하여 동일 그룹 내 게임 상태의 일관성을 보장. 그룹은 IOCP 스레드 위에서 안전하게 동시 실행되도록 설계되어, 코어 수에 비례한 자연스러운 스케일 아웃이 가능함.
