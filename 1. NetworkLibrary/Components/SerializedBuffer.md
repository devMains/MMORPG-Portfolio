...
- 구현 시 직면한 문제 : 네트워크에서 콘텐츠 코드로 버퍼로 메시지를 제공하게 되어 완벽한 분리를 할 수 없다. 따라서 클래스 차원에서 관리를 하여 참조 카운트 기반으로 실질적인 해제를 진행한다. 또한 버퍼가 필요할 때마다 매번 동적 할당/해제를 하게 된다면 속도 또한 느려질 수 있다.
- 해결 방법 : 풀로 관리하여 동적 할당/해제를 매번 하는 것이 아니라 버퍼의 재사용을 통해 성능 향상을 기대한다.
