...
| 항목 | 상세 내용 |
| :--- | :--- |
| **목적** | 전체 게임 서버 시스템의 조감도(Bird's-eye view) 및 서버 간 데이터 흐름 명세 |
...
시스템은 확장성(Scalability)과 관심사 분리(Separation of Concerns)를 위해 역할별로 서버를 분리하여 설계했습니다.
...
*   **Redis (Session Cache):** 로그인 서버에서 발급한 유저의 **내부 Session Key**를 중앙 집중형으로 저장하는 글로벌 인메모리 캐시. 이를 통해 다른 서버(채팅/게임)에서 메모리를 공유하지 않고도 유저의 인증 상태를 빠르게 검증(Stateless)할 수 있습니다.
